...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        NSDictionary *titleattribute = @{NSFontAttributeName: self.titleLabel.font};
        CGSize titleSize = [self.titleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.titleLabel.font.lineHeight) options: NSStringDrawingUsesLineFragmentOrigin attributes:titleattribute context:nil].size;
...
//                                                      lineBreakMode:self.subtitleLabel.lineBreakMode];
        ...
        NSDictionary *subtitleattribute = @{NSFontAttributeName: self.subtitleLabel.font};
        CGSize subtitleSize = [self.subtitleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.subtitleLabel.font.lineHeight) options: NSStringDrawingUsesLineFragmentOrigin attributes:subtitleattribute context:nil].size;
...
//fixed ios 7 bugs
//https://github.com/samvermette/SVPullToRefresh/issues/147
- (void)startAnimating{
    switch (self.position) {
        case SVPullToRefreshPositionTop:
//            
